...
![](png/workflow.png){width=100% fig-align="center" .shaded-border}
...
Our project begins by gathering historical raw data on overnight shelter occupancy using 
Open Data APIs, which is then uploaded to BigQuery using an R script. This includes 
a one-time data upload for historical data from 2022 and ongoing daily updates for 2023. 
To enhance our prediction model, we also collect five-day weather forecasts for the 
Toronto area from the AccuWeather API. This weather information is a key factor in 
predicting shelter needs over the next five days, along with the occupancy data.

---

# __Data Storage__

As mentioned earlier, this raw data in stored in a BigQuery project, in a dataset 
...
for subsequent analysis and predictive modeling. Leveraging DBT (Data Build Tool), 
a set of DBT models are created to convert raw datasets into a refined form, 
stored in a dataset aptly named **clean_data**.
...
1. **Data Cleaning with DBT Models:**
    ...
    it crafts unique primary keys that anchor each data row to a specific shelter location and 
    program offering, thereby enabling precise tracking and referencing within the dataset.
    
    - **`weather_historical_2022_2023`**: Here, historical weather information for the 
    same time frame is merged into the dataset, enriching the shelter data with 
...
2. **Preparing Data for Machine Learning:**
    ...
    daily shelter occupancy figures with corresponding weather metrics. It ensures 
    completeness by employing a 3-day rolling average to impute any gaps in the weather 
    dataset, thereby maintaining the continuity and reliability of the data for 
...
Homes First Society (Org ID 15) and Dixon Hall (Org ID 6). All together, these 3 organizations
have about 40 distinct locations. Again in a later phase, I hope to expand this project to include all locations.

- I also focused on only modeling locations that have been open (in 2022 and 2023) for at least 75% of the year.

- According to the [information](https://open.toronto.ca/dataset/daily-shelter-overnight-service-occupancy-capacity/), 
in the dataset, there are two measures of capacity; **Funding** capacity reports the 
...
with very low error rates; MSE of 0.0648, RMSE of 0.255, MAE of 0.1737.
...
data metrics, which reveal an MSE of 152.845 and an RMSE of 12.36305. The MAE on 
the validation set is 7.166799. The large discrepancy between the training and 
...
but far better than the validation metrics. With a cross-validated RMSE of 0.9291009 
and MAE of 0.3556345, the model demonstrates decent predictive ability on data it 
...
attention. However, such concerns will be systematically addressed in the subsequent 
phase of the project, where model accuracy and generalization become the central focus.
...
Before predictions can be made, an essential data preparation step is conducted 
...
1. **Feature Engineering in BigQuery**: The raw data is processed by the DBT model within BigQuery,  The **`shelter_occupancy_forecast_features`** model applies necessary transformations and engineering to prepare the dataset with features suitable for the predictive model.
...
The deployment of the shelter occupancy forecast into a Shiny application represents 
the culmination of the data pipeline and the predictive modeling process. Shiny 
...
4. **Deployment and Access**: The completed Shiny application is then deployed 
to a server where it can be accessed by users through a web browser. Shiny apps 
can be hosted on various platforms, such as ShinyApps.io, RStudio Connect, or a 
custom server setup, depending on the project's scale and user access requirements.

5. **Maintenance and Updates**: As the last step in the deployment phase, provisions 
are made for the regular updating of the predictive data from BigQuery and for the 
maintenance of the Shiny application to ensure its continued performance and relevance.
...
1. **Accuracy Tracking**:
    ...
    This script can be set to run at specific intervals (e.g., daily, weekly) and 
    update a performance tracking dashboard or report.
    
    - **Statistical Analysis of Predictive Performance**: Include statistical 
    measures such as MAE, RMSE in the script to quantify the accuracy of the predictions.
    
    - **Data Visualization of Performance Metrics**: Enhance the script to produce 
    visualizations that help in quickly identifying trends, deviations, or patterns 
    in the predictive performance over time.
...
3. **Better Model Experiment Tracking**:
    - Incorporate model tracking and experiment loggiing capabilities using a tool 
